    ...
    computed: {
        treeData () {
            ...
            return cloneData.filter(father => {
                let branchArr = cloneData.filter(child => father.listID == child.officeId); //返回每一项的子级数组
                branchArr.length > 0 ? (father.children = branchArr) : ""; //如果存在子级，则给父级添加一个children属性，并赋值
                // return father.officeId == this.options[0].officeId; //返回第一层
                let p = []
                p = cloneData.filter(x => father.officeId == x.listID);
                return p.length == 0;
            });
        }
    },
