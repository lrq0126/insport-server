...
export default {
    ...
    computed: {
        treeData () {
            let cloneData = JSON.parse(JSON.stringify(this.options))
            return cloneData.filter(father => {
                let branchArr = cloneData.filter(child => father.listID == child.officeId)
                branchArr.length > 0 ? (father.children = branchArr) : ""
                return father.listID == '1156154145430876161'
            })
        }
    },
    methods: {
