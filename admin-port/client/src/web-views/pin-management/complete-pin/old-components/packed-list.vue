...
<template>
  <div>
    ...
    <el-table :data="pinData" border v-loading="loading" style="width: 100%">
      ...
      <el-table-column label="打包状态" min-width="130" align="center">
        <template slot-scope="scope">
          <el-tag v-if="scope.row.isPacked == 0" type="info">待打包</el-tag>
          <el-tag v-if="scope.row.isPacked == 1" type="success">已打包</el-tag>
        </template>
      </el-table-column>
      <el-table-column label="发货状态" min-width="130" align="center">
        <template slot-scope="scope">
          <el-tag v-if="scope.row.isShipped == 0" type="info">待发货</el-tag>
          <el-tag v-if="scope.row.isShipped == 1" type="success">已发货</el-tag>
        </template>
      </el-table-column>
      ...
      <el-table-column label="操作" fixed="right" width="340" align="center">
        <template slot-scope="scope">
          <el-button
            ...
            size="mini"
            plain
            @click="printPingMain(scope.row.id)"
            >打印订单</el-button
          >
          ...
          <!-- <el-tag v-if="scope.row.isPacked == 1">
            <el-button
              ...
              size="mini"
              plain
              @click="packingOpen(scope.row.id)"
              >发 货</el-button
            >
          </el-tag> -->
        </template>
...
export default {
  ...
  methods: {
    ...
    pagination(pageObj) {
      if (pageObj.bool) {
        this.pageInfo.pageNumber = pageObj.pageNumber;
        this.queryLiist();
      } else {
        this.pageInfo.pageSize = pageObj.pageSize;
        this.queryLiist(1);
      }
    },
     ...
     */
    checkDetail(pinId) {
      this.$router.push({ name: "PackedDetail", query: { id: pinId } });
    },
