    ...
    <div class="filter-container">
        ...
        <el-table :data="tableData"
            ...
            <el-table-column prop="code"
                             label="校卡编码"></el-table-column>
            <el-table-column prop="personnelName"
                             label="持卡人"></el-table-column>
            <el-table-column label="校卡状态">
                ...
                <template slot-scope="scope">
                    ...
                    <el-dropdown v-show="hasAuthority('systemRoleEdit')"
                        ...
                        <el-dropdown-menu slot="dropdown">
                            <el-dropdown-item command="issue">发放</el-dropdown-item>
                            <el-dropdown-item command="recycling">回收</el-dropdown-item>
...
import {
    getSchoolCardManage,
    getSchoolCardManageDetail,
    removeSchoolCardManage,
    issueSchoolCardManage,
    recyclingSchoolCardManage,
    invalidSchoolCardManage
} from '@/api/basic-info'
    ...
    methods: {
       ...
       */
        handleSearch (page) {
            ...
            this.loading = true
            getSchoolCardManage(this.pageInfo).then(res => {
        ...
        */
        ...
        handleModal (id) {
            if (id) {
                this.saving = true
                getSchoolCardManageDetail(id).then(res => {
                    if (res.code === 0) {
        ...
        handleRemove (id) {
            this.$confirm('确定删除吗？', '提示', {
                ...
                removeSchoolCardManage(id).then(res => {
                    if (res.code === 0) {
        ...
        handleClick (name, row) {
            switch (name) {
                case 'issue':
                    if (row.status != '1') {
                        issueSchoolCardManage(row.identityId).then(res => {
                            if (res.code === 0) {
                                this.pageInfo.page = 1
                                this.$message({
                                    message: "发放校卡成功",
