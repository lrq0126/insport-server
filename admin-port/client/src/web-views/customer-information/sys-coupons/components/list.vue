  ...
  <div>
    ...
    <el-table :data="tableData" border v-loading="loading" style="width: 100%">
      ...
      <el-table-column label="操作" fixed="right" width="350" align="center">
        <template slot-scope="scope">
          <el-button :disabled="scope.row.isEnable != 1" type="success" size="mini" plain @click="giveModal(scope.row)"
            >分发</el-button
          >
          <el-button type="info" size="mini" plain @click="editModal(scope.row)"
            >编辑</el-button
          >
          <el-button
            type="warning"
    ...
    <coupons-detail-view
      ref="couponsDetailView"
      @updateList="handleSearch(1)"
    />

    <give-coupons-view ref="giveCouponView" />
  </div>
</template>
...
import CouponsDetailView from "./coupons-detail";
import GiveCouponsView from "./give-coupons";
...
export default {
  ...
  methods: {
     ...
     */
    createModal() {
      this.$refs["couponsDetailView"].handCreate();
    },
     ...
     */
    editModal(row) {
      this.$refs["couponsDetailView"].handEdit(row.id);
    },
     ...
     */
    giveModal(row) {
      this.$refs["giveCouponView"].openGiveCoupons(row);
    },
  },
  mounted() {
    this.handleSearch();
