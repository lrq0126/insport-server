    ...
    <div>
        <el-form inline label-width="120px">
            <el-form-item label="待办标题">
                <el-input placeholder="请输入待办标题" v-model="pageInfo.title"/>
            </el-form-item>
        ...
        <el-table :data="backlogData" width="100%" v-loading="loading" border>
            ...
            <el-table-column label="操作" align="center" width="320px">
                <template slot-scope="scope">
                    ...
                    <el-button size="mini" 
                        v-if="scope.row.backlogStatus == 0 && scope.row.backloggerId == loginUserId" 
                        type="primary"
                    ...
                    <el-button size="mini" 
                        v-if="scope.row.backlogStatus == 1 && scope.row.backloggerId == loginUserId" 
                        type="success"
                        @click="upBacklogStatus(scope.row.id, 2)">完 成</el-button>
                    <el-button size="mini" type="danger" plain @click="deleteBacklog(scope.row.id)">删 除</el-button>
                </template>
            </el-table-column>
        </el-table>
