...
export default {
  data() {
    return {
      ...
      title: ["orderNumber", "packNumber", "routeName"],
  ...
  methods: {
    CopyExcelToTable() {
      ...
      resultData.forEach((ele) => {
        if (!ele.orderNumber) {
          this.$message.error("存在不正确的订单号数据,请检查");
          change = false;
        }

        if (!numRe.test(ele.packNumber)) {
          this.$message.error("存在不正确的包裹数量数据,请检查");
          change = false;
        }
      });
      ...
      // 关闭显示框，情况text表的数据
      if (change) {
        this.$emit("updateDetailList", resultData);
        this.dialog = false;
