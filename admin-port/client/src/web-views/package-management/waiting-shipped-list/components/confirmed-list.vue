    ...
    <div>
        ...
        <el-form ref="searchForm"
            ...
            <el-form-item label="路线类型">
                <el-select v-model="pageInfo.routeType" clearable
                           placeholder="请选择">
                    <el-option label="海运"
                               value="2">
                    </el-option>
        ...
        <el-table :data="tableData"
            ...
            <el-table-column label="操作"
                             ...
                             align="center">
                <template slot-scope="scope">
                    <el-button v-if="scope.row.packType == 2 || scope.row.packType == 3"
                               type="info"
                               size="mini"
                               plain
                    ...
                    <el-button v-if="scope.row.packType == 3"
                               ...
                               size="mini"
                               plain
                               @click="handleFapiao(scope.row.id)">编辑发票</el-button>
                    
                    <el-button v-if="scope.row.packType == 3 && filterPermissions('确认发货')"
                               type="success"
                               size="mini"
                               plain
                    ...
                    <el-button type="primary"
                               size="mini"
                               plain
                               @click="amountChange(scope.row)">金额变更</el-button>
                    <el-button v-if="scope.row.packType == 2 || scope.row.packType == 3"
                               type="warning"
                               size="mini"
                               plain
                    ...
                    <el-button v-if="scope.row.packType == 7 && filterPermissions('确认发货')"
                               type="danger"
                               size="mini"
                               plain
                               @click="handleCaibao(scope.row.id,scope.row.businessNumber)"
                               :disabled="scope.row.roleCode != 'all' && hasAuthority('systemRoleEdit')?false:true">确认拆包</el-button>
                    <el-button v-if="scope.row.packType == 7 && filterPermissions('确认发货')"
                               type="danger"
                               size="mini"
                               plain
                               @click="handleCancelCaibao(scope.row.id)"
                               :disabled="scope.row.roleCode != 'all' && hasAuthority('systemRoleEdit')?false:true">取消拆包</el-button>
                    <el-button v-if="scope.row.packType == 2"
                               type="danger"
                               size="mini"
                               @click="handleOrderVoided(scope.row.businessNumber,scope.row.id)">订单作废</el-button>
                </template>
            </el-table-column>
        </el-table>
        ...
        <el-dialog :visible.sync="confirmDeliveryDialog" width="40%" title="确认发货">
            <el-form label-width="100px">
                <el-form-item label="订单号：">
                    <span style="font-size: 24px">{{confiemDeliveryPack.orderNumber}}</span>
                </el-form-item>
                <el-form-item label="转单号：">
                    <el-input v-model="confiemDeliveryPack.transitNumber" 
                    placeholder="请输入转单号(无转单号可不填)"
                    class="transitNumber-class"/>
                </el-form-item>
                <el-form-item>
                    <el-button type="primary" style="margin-left: 20%; margin-top: 30px" @click="confirmDelivery">确认发货</el-button>
                    <el-button type="info" plain @click="confirmDeliveryDialog = false">取 消</el-button>
                </el-form-item>
            </el-form>
        </el-dialog>
...
export default {
    ...
    methods: {
        ...
        amountChange(rowData){
            ...
            this.$refs['amountChangeModelView'].openDialog(data);
        },
       ...
       */
        ...
        */
        ...
        handlePackingDetail (businessNumber, id) {
            this.$router.push({ name: 'WaitingShippedDetail', query: { businessNumber: businessNumber, id: id } })
        },
        ...
        handleModal (data) {
            ...
            this.confiemDeliveryPack.id = data.id
            this.confiemDeliveryPack.orderNumber = data.orderNumber
        ...
        confirmDelivery(){
                ...
                this.$confirm('确定发货吗？', '提示', {
                    ...
                    confirmDelivery(this.confiemDeliveryPack).then(res => {
                        if (res.code === 100) {
        ...
        * @return {type} {description}
        */
        handleCancelCaibao (id) {
            this.$confirm('确定取消拆包吗？', '提示', {
                confirmButtonText: '确定',
                cancelButtonText: '取消',
                type: 'warning'
            }).then(() => {
                cancelUnpackingWaitingArea(id).then(res => {
                    if (res.code === 100) {
                        this.$message({
                            message: "取消拆包成功",
                            type: "success",
