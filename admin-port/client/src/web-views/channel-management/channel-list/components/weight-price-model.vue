...
export default {
  data() {
    return {
      ...
      title: ["startWeight", "endWeight", "price", "remarks"],
  ...
  methods: {
    CopyExcelToTable() {
      ...
      resultData.forEach((ele) => {
        if (!numRe.test(ele.startWeight)) {
          this.$message.error("存在不正确的起始重量数据,请检查");
          change = false;
        }

        if (!numRe.test(ele.endWeight)) {
          this.$message.error("存在不正确的结尾重量数据,请检查");
          change = false;
        }
