...
export default {
  data() {
    return {
      ...
      title: ["startVolume", "endVolume", "price", "remarks"],
  ...
  methods: {
    CopyExcelToTable() {
      ...
      resultData.forEach((ele) => {
        if (!numRe.test(ele.startVolume)) {
          this.$message.error("存在不正确的起始体积数据,请检查");
          change = false;
        }

        if (!numRe.test(ele.endVolume)) {
          this.$message.error("存在不正确的结尾体积数据,请检查");
          change = false;
        }

        if (!numRe.test(ele.price)) {
          this.$message.error("存在不正确的单位价格数据,请检查");
          change = false;
        }
      });
