...
<template>
  <div class="model-shipping-view">
    ...
    >
      <el-tab-pane label="渠道信息详情" name="list">
        <el-form
          ref="form"
          :model="formItem"
          :rules="formItemRules"
                    ...
                    <el-table-column label="起始体积/m³" align="center">
                      <template slot-scope="scope">
                        <el-form-item
                          ...
                          :prop="
                            'packagePrice.' + scope.$index + '.startVolume'
                          "
                          :rules="{
                            required: true,
                            message: '起始体积不能为空',
...
export default {
  ...
  data() {
    return {
      ...
      formItemRules: {
        ...
        // startVolume: [
        //     { required: true, message: '起始体积不能为空', trigger: 'blur' },
        // ],
        // endVolume: [
        //     { required: true, message: '结尾体积不能为空', trigger: 'blur' },
        // ],
        // price: [
        //     { required: true, message: '单位价格不能为空', trigger: 'blur' },
        // ],
  ...
  methods: {
     ...
     */
    handleSubmit(from) {
      // this.formItem.smallPackagePrice.forEach(ele => {  // 给JSON添加公共数据
      //     ele.greaterWeight = this.formItem.greaterWeight
      //     ele.forkliftFee = this.formItem.forkliftFee
      //     ele.smallWeightCarryType = this.formItem.smallWeightCarryType
      // });
      // this.formItem.packagePrice.forEach(ele => {  // 给JSON添加公共数据
      //     ele.volConversion = this.formItem.volConversion
      //     ele.sensitivePrice = this.formItem.sensitivePrice
      //     ele.cubeUpper = this.formItem.cubeUpper
      //     ele.greaterWeight = this.formItem.greaterWeight
      //     ele.forkliftFee = this.formItem.forkliftFee
      //     ele.firstLength = this.formItem.firstLength
      //     ele.secondLength = this.formItem.secondLength
      //     ele.upperLimitWeight = this.formItem.upperLimitWeight
      // });
      this.$refs[from].validate((valid) => {
        if (valid) {
          this.saving = true;
          shippingUpdataChannelList(this.formItem)
            .then((res) => {
              if (res.code === 100) {
                this.$message({
                  message: "海运渠道编修改成功",
    ...
    handleReset(from) {
      ...
      // this.formItem.smallPackagePrice = [{
      //     greaterWeight: '',
      //     forkliftFee: '',
      //     smallWeightCarryType: '',
      //     smallStartWeight: '',
  ...
  mounted() {
    ...
    this.formItem.transportType = String(obj.transportType);
    ...
    if (obj.routePrices.length > 0) {
      console.log("价格内容：", obj.routePrices);
      this.formItem.packagePrice = obj.routePrices;
      console.log("价格内容：", this.formItem.packagePrice);
    }
  },
};
</script>
