...
import { getRechargeRecord, rechargeSave } from '@/api/financial-management/top-up-records'
...
export default {
    ...
    methods: {
        ...
        handleSearch (id) {
            this.pageInfo.id = id;
            getRechargeRecord(this.pageInfo).then(res => {
                if (res.code === 100) {
                    this.formItem.id = res.content.id
                    this.formItem.customerName = res.content.customerName
                    this.formItem.registrationTime = res.content.createTime
                    this.formItem.phone = res.content.phoneNumber
                    this.formItem.customerNo = res.content.customerNo
                    this.formItem.loginName = res.content.loginName
                    this.formItem.amount = res.content.amount
                    this.formItem.channel = res.content.channel
        ...
        handleSubmit (from) {
            this.$refs[from].validate((valid) => {
                if (valid) {
                    ...
                    this.saving = true
                    rechargeSave(this.formItem).then(res => {
                        if (res.code === 100) {
                            this.$message({
                                message: '修改成功',
                                type: 'success',
