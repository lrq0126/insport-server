...
const store = new Vuex.Store({
  ...
  mutations: {
	loginOperation(state){
		uni.setStorageSync("isOperation", "true");
	},
	
    login(state) {
		let isOperation = uni.getStorageSync("isOperation")
		if(isOperation){
			state.hasUserLogin = true;
		}
    },
    logout(state) {
      state.hasUserLogin = false;
	  uni.removeStorageSync('isOperation');
    },
