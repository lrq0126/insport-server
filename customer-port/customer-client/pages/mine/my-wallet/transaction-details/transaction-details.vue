            ...
            <view class="subordinates">
                <view class="subordinates_ul">
                    ...
                    <view class="subordinates_li"
                          :class="{'subordinates_li_on' : pageInfo.costType == 2 }"
                          @click="handleQuerying('2')">扣费记录</view>
                    <view class="subordinates_li"
                          :class="{'subordinates_li_on' : pageInfo.costType == 3 }"
                          @click="handleQuerying('3')">退款记录</view>
                </view>
            </view>
            <view class="bg_bs"
                  v-if="tableData.length > 0">
                <view class="transaction_detail_03">
                    <view align="right">您当前的总消费金额为：<font color="#FF0000"
                ...
                <view class="transaction_detail_03"
                      ...
                      :key="index"
                      @click="handleGo('../recharge-record/record-details/record-details', item)">
                    <!-- <view class="redx">未读</view> -->
                    <!-- <view class="redx">已读</view> -->
                    <view>客户名称：{{item.customerName}}
                        <text class="transaction_detail_04"
                              style="color: green;"
                              v-if="item.costType == '2'">【扣费成功】</text>
                        <text class="transaction_detail_04"
                              style="color: green;"
                              v-if="item.costType == '3'">【退款成功】</text>
                    </view>
                    <view>运单号：{{item.businessNumber}}</view>
                    <view>订单号：{{item.orderNumber}}
                        <text class="transaction_detail_04"
                              v-if="item.costType == '2'"
                              style="color: #666;">费用金额：<text style="color: red;font-weight: bold;">￥{{item.amount}}</text></text>
                        <text class="transaction_detail_04"
                              v-if="item.costType == '3'"
                              style="color: #666;">退款金额：<text style="color: red;font-weight: bold;">￥{{item.amount}}</text></text>
                    </view>
                </view>
            </view>

            <view v-if="tableBool"
                  style="text-align: center;margin: 15px 0;color: #666;">暂无记录信息</view>
            <!-- 页码模块 -->
