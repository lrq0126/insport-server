    ...
    <view class="pending-payment">
        ...
        <view class="package_stock">
            <form method="post"
                ...
                <!-- <input placeholder="请输入重量(数字)单位KG" name="keyword" type="number" v-model="pageInfo.weight" id="keyword" style="width: 93%;margin-top: 20upx;margin-left: 10upx;"
...
import {
    pinWeightQuotationInquiryList,
    volumeQuotationInquiryList,
    getCountriesList
} from "@/api/quotation-inquiry"
import {
    getDeliveryCompany
...
export default {
    ...
    onLoad (option) {
        this.pageInfo.weight = option.weight;
    ...
    methods: {
        ...
        submit () {
            ...
            this.$Navigate.navigateBack(1, {
                'transportChannels': this.selectData
            })
        },
        ...
        handleSearch () {
            ...
            Promise.all([p2]).then(res => {
                if (res[0][1].data.code == 200) {
                    // this.tableData = res[0][1].data.content.routePrices ? res[0][1].data.content.routePrices : [];
                    ...
                    this.countriesData = res[0][1].data.content ? res[0][1].data.content.country : []; // 国家
                    this.pageInfo.countryId = res[0][1].data.content.country[0].id
                    this.pageInfo.weightCountryId = res[0][1].data.content.country[0].id
                    this.handleQueryingWeight(1)
        ...
        handleQueryingWeight (page) {
            ...
            if (this.pageInfo.weight) {
                this.showloading()
                this.pageInfo.routeType = 1
                this.pageInfo.transportType = this.pageInfo.weightTransportType
                this.pageInfo.countryId = this.pageInfo.weightCountryId
				this.pageInfo.isPin = 1;
                pinWeightQuotationInquiryList(this.pageInfo)
                    .then(res => {
