<template>
	<view :class="{ 'uni-card--full': isFull === true || isFull === 'true', 'uni-card--shadow': isShadow === true || isShadow === 'true' }" class="uni-card uni-card-box" @click="onClick">
		...
		<view v-if="mode === 'basic' && title" class="uni-card__header">
			<view v-if="thumbnail" class="uni-card__header-extra-img-view">
				<!-- <image :src="thumbnail" class="uni-card__header-extra-img" style="width: 46upx;height: 46upx;margin-right: 10upx;" /> -->
				<dn-icon :type="thumbnail" size="25" color="#e65b07"></dn-icon>
			</view>
...
<style>
	...
	/* 修改通知公告行间距 */
	.uni-card-box .uni-card__header-extra-img-view {
		line-height: 0;
		position: relative;
		top: 3px;
	}
